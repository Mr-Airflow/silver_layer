...
# =============================================================================
# MultiSourceReader — get_sources_config (no Spark required)
# =============================================================================

class TestMultiSourceReader:
    def _make_reader(self, cfg):
        from unittest.mock import MagicMock
        from utils.silver_io import MultiSourceReader
        return MultiSourceReader(spark=MagicMock(), pipeline_cfg=cfg)

    def test_single_source_table_normalised(self, sample_single_source_config):
        reader = self._make_reader(sample_single_source_config)
        sources = reader.get_sources_config()
        assert len(sources) == 1
        assert sources[0]["alias"]      == "source"
        assert sources[0]["table"]      == "bronze.customers"
        assert sources[0]["is_primary"] is True

    def test_multi_source_config_preserved(self, sample_source_tables_config):
        reader = self._make_reader(sample_source_tables_config)
        sources = reader.get_sources_config()
        assert len(sources) == 2
        aliases = [s["alias"] for s in sources]
        assert "customers" in aliases
        assert "branches"  in aliases

    def test_is_multi_source_false_for_single(self, sample_single_source_config):
        reader = self._make_reader(sample_single_source_config)
        assert reader.is_multi_source() is False

    def test_is_multi_source_true_for_multi(self, sample_source_tables_config):
        reader = self._make_reader(sample_source_tables_config)
        assert reader.is_multi_source() is True

    def test_no_source_raises(self):
        reader = self._make_reader({})
        with pytest.raises(ValueError, match="source_table"):
            reader.get_sources_config()

    def test_default_primary_is_first_entry_when_none_flagged(self):
        cfg = {
            "source_tables": [
                {"alias": "a", "table": "bronze.a"},
                {"alias": "b", "table": "bronze.b"},
            ]
        }
        reader = self._make_reader(cfg)
        sources = reader.get_sources_config()
        assert sources[0].get("is_primary") is True
        assert sources[1].get("is_primary", False) is False

    def test_primary_source_name(self, sample_source_tables_config):
        reader = self._make_reader(sample_source_tables_config)
        assert reader.primary_source_name() == "bronze.customers"
